...
- **Built so far:** the full Core engine (models, zip+JSON persistence, propagation model, Kriging interpolation, AP placement, CSV/JSON/PDF export, coverage-grid calculation), the app shell (DI, NavigationView, Mica, HomePage), the New Survey wizard, and the Workspace page (`FloorPlanCanvas`: walls/test points/APs/heatmap rendering, tool-mode editing with auto-save, Coverage/Recommendations/Adapter side panel). Still open: WLAN scanning (native `wlanapi.dll` P/Invoke — see the plan's research notes on why the WinRT `Windows.Devices.WiFi` API won't work unpackaged), the guided measurement walk, and live adapter/band detection — all bundled together since the walk's capture step is itself a WLAN scan call.
