...
**Amended (2026-09-03):** the WLAN link diagnostics feature added a *second* category of network activity that this ADR did not anticipate and the README bullet did not cover — `NetworkHealthService` pings the adapter's own default gateway and times one DNS resolution of `www.msftconnecttest.com`. Both are inherent to what a "why is my WiFi slow" check measures, and neither is optional in the way the version check is: a user who runs diagnostics is asking for exactly this. They are, however, still outbound traffic (the DNS query leaves the machine for whichever resolver the adapter is configured with), so they are now named explicitly in the README rather than left implied. No separate toggle: unlike the launch-time version check, these only happen while the user is actively running or monitoring diagnostics, so the action itself is the consent.
